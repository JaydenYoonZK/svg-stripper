...
- The name seal is now a proper square chop, sitting level with the wordmark, and its characters and inner frame are cut clean through the chartreuse field. Whatever the bar shows behind them fills the strokes, so the seal belongs equally to the light and dark themes, and the brand name grew a touch to keep the pair balanced.

## [1.7.8] - 2026-08-16

### Added

- The brand link in the navigation now carries Jayden's Chinese name, 袁正凯, set upright in a small vertical seal beside the wordmark. It wears the site's chartreuse and ink, the same pairing the buttons use, so it reads like a stamp pressed in the key colors.
...
- The green pill that floated over the status bar on an iPhone is gone. It was the skip-to-content link all along, resting just above the viewport's top edge, which is exactly the zone iOS paints behind the clock; the iOS Simulator caught it in the act. It parks off to the side now, and still slides in the moment it takes keyboard focus.

### Changed

- The menu bar rides position fixed instead of sticky, so a hard fling cannot lag it a frame, and it wears safe-area padding so it owns the strip under the status bar when the site is saved to the home screen. The shield strips from the last two releases are removed: nothing inside a page can paint in the status-bar zone, so neither of them ever did anything. The faint page content showing through behind the status bar while scrolling is Safari's own edge-to-edge design, apple.com shows the very same thing, and this page no longer fights it.
...
- On iPhones, scrolling could flash page content in the strip under the clock and battery, above the sticky menu. The cover for that strip used to live inside the blurred bar, where iOS never drew it, so it now sits just outside the bar and actually shows up.

## [1.7.3] - 2026-08-15

### Changed

- The copyright sentence stands alone again, and the version link now sits on its own line right beneath it.
